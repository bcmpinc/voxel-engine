    ...
    if (depth>=0 && bound[1] + bound[0] <= 2<<SCENE_DEPTH) {
        if (octnode < 0xff000000) {
            ...
            for (int k = 0; k<8; k++) {
                ...
                count_oct++;
                if (traverse(quadnode, root[octnode].child[j], new_bound, dx, dy, dz, dgtz, pos + (DELTA[i]<<depth), depth-1)) return true;
            }
        } else {
            // Duplicate leaf node
            v4si octant = -(pos<0);
            ...
            for (int k = 0; k<7; k++) {
                ...
                if ((gtz[0] & gtz[1] & gtz[2] & gtz[3]) == 0) continue; // frustum occlusion
                count_oct++;
                if (traverse(quadnode, octnode, new_bound, dx, dy, dz, dgtz, pos + (DELTA[i]<<depth), depth-1)) return true;
            }
        }
        return false;
    } else {
